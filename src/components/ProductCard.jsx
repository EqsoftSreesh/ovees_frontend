...
import { Minus, Plus, ShoppingCart, Eye, Heart, Zap, Star, Check } from 'lucide-react'
...
const ProductCard = ({ product, onProductClick, onAddToCart, cartItems = [] }) => {
  ...
  const discountPercent = product.offer_price 
    ? Math.round(((product.normal_price - product.offer_price) / product.normal_price) * 100)
    : 0
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-teal-50/0 to-yellow-50/0 group-hover:from-teal-50/50 group-hover:to-yellow-50/50 transition-all duration-500 pointer-events-none z-10"></div>

      {/* Image Container */}
      <div className="relative cursor-pointer bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden" onClick={onProductClick}>
        {/* Image */}
        ...
        <div className={`absolute top-3 right-3 flex flex-col gap-2 z-20 transition-all duration-300 ${
          ...
          <button
            className="bg-white/90 backdrop-blur-sm p-2.5 rounded-full shadow-lg hover:bg-pink-500 hover:text-white transition-all duration-300 group/btn"
          >
          ...
          {product.offer_price ? (
            <div className="space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-2xl font-black text-gray-900">₹{product.offer_price}</span>
                <span className="text-sm text-gray-400 line-through">₹{product.normal_price}</span>
              </div>
            ...
            <span className="text-2xl font-black text-gray-900">₹{product.normal_price || product.price}</span>
          )}
        </div>
